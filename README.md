...
| 第一时间获取更新、AI + 法律实战案例 | 加微信进读者群 · 技术交流 · 项目咨询 |

🙏 **如果律鉴对你的工作有帮助，欢迎赞赏支持作者持续维护**（赞赏备注「PRC-Law」可加入优先反馈名单，新功能/法律数据需求优先响应）。
...
| **公司大法务 / 大所** | 威科先行企业版 + 律鉴做内部分析加速 |
| **AI 能力强 / 极客律师** | 律鉴单独够用（双源走元典+法宝 API），不订阅北大法宝 |
| **不写代码的法务** | 律鉴装在 WorkBuddy（零命令），拖拽即可用 |

📊 **完整对比矩阵**：[docs/vs-competitors.md](docs/vs-competitors.md) ← 含 5 个真实场景、双源核验、离线能力、价格 ROI
...
![demo-nd1](docs/assets/demo-nd1.svg)

> 用户问: 2007 年消费者购车欺诈案,赔偿标准用哪个版本?
> 律鉴答: 自动锁定 **1993 版《消法》第 49 条 (退一赔一)**,不是 2013 版 (退一赔三)。
> **3 倍赔偿差异直接影响和解金额。** 5.3 秒完成,1 API 调用,¥0 (prc-law-data 命中)。

### Demo · SaaS 合同审查 6 维评分 (ND3)

![demo-nd3](docs/assets/demo-nd3.svg)

> 6 Agent 并行审查 → 5 维评分 (合规/财务/防御/履行/商业) → 高风险自动标红。
> **3 分 12 秒出报告**, 比人工审查快 50 倍。

### Demo · 内部调查 24h 响应 (ND5)

![demo-nd5](docs/assets/demo-nd5.svg)

> 员工被举报收受回扣 → 证据评估 → 法律检索 → 风险矩阵 → 处置建议 → 文书生成。
> **风险矩阵 4 维可视化**,推荐方案自动标亮。2 分 45 秒完成。

**完整 demo 列表**: [docs/DEMOS.md](docs/DEMOS.md) (6 个端到端真实案例)
