...
- 一句话定位 + 价格表
- 14 维度对比矩阵(律鉴 vs 北大法宝 vs 威科先行)
- 5 个真实场景(消法版本 / 数据深度 / 团队协作 / 离线 / 审查效率)
- 4 类律师选型决策树
- 坦诚告知「律鉴不能替代的部分」(案例库/国际法/期刊/律所管理)
- 价格 ROI 总结

**`.github/ISSUE_TEMPLATE/`** (新) — 3 个模板:
- `lawyer-feedback.yml` — 律师/法务反馈专用通道,带评分表(法律准确性/法条时效/类案匹配/文书可用/响应速度/vs 商业库),选填背景(执业领域/年限/律所规模),联系渠道(微信号/优先反馈名单/公开致谢)
- `bug-report.yml` — 通用 bug 报告,带客户端/OS/Python/branch 元数据 + 影响严重度分级
- `feature-request.yml` — 功能请求,带替代方案调研(北大法宝/威科/法狗狗/DeepLaw) + 优先级(付费意愿)

**`docs/assets/banner-social.svg`** (新) — 1200x630 GitHub OG 标准尺寸社交分享 banner:
- 主标题「律鉴」+天平图标 + 关键数字(92%/152/18,636/¥0)
- 公众号引导位「关注搞 AI 的陈老师」(右侧)
- 底部品牌信息 github.com/godlockin/PRC-Law

**README.md**: 顶部新增 `## ⚖️ 我适合用律鉴吗?` 5秒决策表 + 链接到 `vs-competitors.md`
